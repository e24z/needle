...
class MLXSwePrunerBackend:
    ...
    def _record_backbone_provenance(self, backbone: ResolvedBackbone) -> None:
        from needle_worker.model_download import augment_model_provenance
...
def _resolve_model_dir() -> str:
    ...
    Hugging Face cache.
    """
    explicit = os.environ.get("NEEDLE_MODEL_DIR")
    if explicit:
        return explicit
    from needle_worker.model_download import download_model_snapshot

    repo = os.environ.get("NEEDLE_MODEL", "ayanami-kitasan/code-pruner")
    revision = os.environ.get("NEEDLE_MODEL_REVISION")
